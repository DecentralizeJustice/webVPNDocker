<template>
  ...
  <button class="inline-block py-3 px-6 my-2 leading-none text-white
   ...
   <section class="bg-grey" v-if="unlocked">
    <section class="py-8">
  <div class="container px-4 mx-auto">
    <div class="pt-4 bg-white shadow rounded">
      <div class="flex px-6 pb-4 border-b justify-center">
        <h3 class="text-xl font-bold text-black">VPN Connections</h3>
      </div>
      <div class="p-4 overflow-x-auto">
        <table class="table-auto w-full">
          <thead>
            <tr class="text-s text-gray-500 text-center">
              <th class="pb-3 font-medium">Device Name</th>
              <th class="pb-3 font-medium">QR Code</th>
              <th class="pb-3 font-medium">Download Config</th>
              <th class="pb-3 font-medium">Delete</th>
            </tr>
          </thead>
          <tbody class="text-black">
            <tr class="text-xs bg-gray-50">
              <td class="font-medium">Blue Phone</td>
...
async function getUser() {
  try {
    ...
    src.value = await QRCode.toDataURL(sodium.to_string(decrypted))
    unlocked.value = true
